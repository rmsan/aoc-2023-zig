...
pub fn setup_day(
    ...
    const exe = b.addExecutable(.{ .name = path, .root_source_file = b.path(root_src), .target = target, .optimize = mode });
    ...
    const exe_test = b.addTest(.{ .root_source_file = b.path(root_src), .target = target, .optimize = mode });
